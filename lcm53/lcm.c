...
void LCM_read_param (int argc, char *argv[], PROBLEM *PP){
  ...
  if ( strchr (argv[c], 'C') ){ PP->problem |= PROBLEM_CLOSED; PP->TT.flag2 |= TRSACT_INTSEC; }
  else if ( strchr (argv[c], 'F') ){ PP->problem |= PROBLEM_FREQSET; II->flag |= ITEMSET_ALL; }
  else if ( strchr (argv[c], 'M') ){ PP->problem |= PROBLEM_MAXIMAL; PP->TT.flag2 |= TRSACT_UNION;}
  ...
  while ( argv[c][0] == '-' ){
    switch (argv[c][1]){
      case 'L': if ( !strcmp (argv[c], "-LAMP") ){
        // topk.base: #transactions, including empty-ones   topk.end:   
        if ( II->flag2 & ITEMSET_LAMP ){
          II->topk.base = atoi(argv[c+1]);
        } else {
      ...
      break; case 'K': if ( PP->problem & PROBLEM_MAXIMAL )
        ...
        II->topk_k = (LONG)atof(argv[c+1]);
      break; case 'm': PP->TT.pfname = argv[c+1];
      break; case 'M': PP->TT.pfname = argv[c+1]; PP->TT.flag2 |= TRSACT_WRITE_PERM;
      ...
      break; case 'f': if ( !RANGE (0, II->prob_lb = atof(argv[c+1]), 1) )
         error_num("ratio has to be in [0,1]",atof(argv[c+1]), EXIT);
         II->flag |= ITEMSET_RFRQ; f++;
      break; case 'F': if ( !RANGE (0, II->prob_ub = atof(argv[c+1]), 1) )
         error_num("ratio has to be in [0,1]",atof(argv[c+1]), EXIT);
         II->flag |= ITEMSET_RINFRQ; f++;
      break; case 'i': II->target = atoi(argv[c+1]);
      break; case 'a': if ( !RANGE (0, II->ratio_lb = atof(argv[c+1]), 1) )
         error_num("ratio has to be in [0,1]",atof(argv[c+1]), EXIT);
         II->flag |= ITEMSET_RULE_FRQ; f|=1;
      break; case 'A': if ( !RANGE (0, II->ratio_ub = atof(argv[c+1]), 1) )
         error_num("ratio has to be in [0,1]",atof(argv[c+1]), EXIT);
         II->flag |= ITEMSET_RULE_INFRQ; f|=1;
      break; case 'r': if ( !RANGE (0, II->ratio_lb = atof(argv[c+1]), 1) )
         error_num("ratio has to be in [0,1]",atof(argv[c+1]), EXIT);
         II->flag |= ITEMSET_RULE_RFRQ; f|=2;
      break; case 'R': if ( !RANGE (0, II->ratio_ub = atof(argv[c+1]), 1) )
         error_num("ratio has to be in [0,1]",atof(argv[c+1]), EXIT);
         II->flag |= ITEMSET_RULE_RINFRQ; f|=2;
